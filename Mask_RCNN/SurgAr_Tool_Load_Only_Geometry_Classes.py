...
class CustomConfig(Config):
    ...
    NUM_CLASSES = 1 + 3  # Background + (midline, tip point, edlines)
...
class CustomDataset(utils.Dataset):
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        classID = []
        clss_names = info["polygons"]["classTitle"]
        for cl in range(len(clss_names)):
            # add this to remove geometric primitives from the dataset 
            if clss_names[cl] == 'mid-line':
                classes.append(clss_names[cl])
            elif clss_names[cl] == 'edge-line':
                classes.append(clss_names[cl])  
            elif clss_names[cl] == 'tool-tip':
                classes.append(clss_names[cl])  
            else:
                continue
        ...
        mask_lines = np.zeros((info["height"], info["width"]),
                        dtype=np.uint8) 
        mask_lines_temp = np.zeros([info["height"], info["width"], len(classes)],
                        dtype=np.uint8)                               
